...
The cask is `tandem-repos` because homebrew-cask's own `tandem` is an unrelated app
(a virtual office), and an unqualified `brew install --cask tandem` gets that one.
...
`Casks/tandem-repos.rb` is written by the
